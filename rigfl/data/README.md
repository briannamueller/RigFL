...
CIFAR-10, CIFAR-100, Tiny ImageNet, FEMNIST, and PaySim fraud detection. The image
dataset sample limits keep initial runs small; set the per-client limits to `null`
to retain all samples assigned to each client.
...
`femnist`, and `paysim_fraud`. The `image` and `numeric` transforms support custom
datasets that need only direct tensor conversion; `auto` infers between those two
forms from the input column.
