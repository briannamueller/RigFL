...
The BioSilo backend loads a partition that was previously generated by BioSilo.
RigFL does not repartition or copy its data.

```yaml
datasets:
  my_biomedical_dataset:
    backend: biosilo
    source_dataset: TCGA
    partition: <biosilo-partition-id>
    data_root: data
    validation_fraction: 0.2
```

`partition` identifies the exact BioSilo partition to use. `data_root` is
optional when BioSilo's default data root or `BIOSILO_DATA_ROOT` is already
configured. BioSilo supplies each client's training and test data; RigFL derives
the validation set from the training data. When BioSilo supplies subject or
patient group IDs, whole groups are assigned to either training or validation so
the same individual cannot appear in both.

BioSilo must be installed in the environment running RigFL. Generate its
partition with `biosilo.generate(...)` before starting the experiment; the
`rigfl.data.generate` command applies only to Flower sources.
